        ...
        <div class="col-md-12">
            @can('traveler_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.travelers.create') }}">
                            ...
                            <tbody>
                                @foreach($travelers as $key => $traveler)
                                    <tr data-entry-id="{{ $traveler->id }}">
                                        ...
                                        <td>
                                            @can('traveler_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.travelers.show', $traveler->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('traveler_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.travelers.edit', $traveler->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('traveler_delete')
                                                <form action="{{ route('frontend.travelers.destroy', $traveler->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('traveler_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.travelers.massDestroy') }}",
