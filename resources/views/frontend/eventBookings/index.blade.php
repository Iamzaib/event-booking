        ...
        <div class="col-md-12">
            @can('event_booking_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.event-bookings.create') }}">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="table-responsive">
                        <table class=" table table-bordered table-striped table-hover datatable datatable-EventBooking">
                            <thead>
                                <tr>
                                    <th>
                                    ...
                                    <tr data-entry-id="{{ $eventBooking->id }}">
                                        ...
                                        <td>
                                            @can('event_booking_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.event-bookings.show', $eventBooking->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('event_booking_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.event-bookings.edit', $eventBooking->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('event_booking_delete')
                                                <form action="{{ route('frontend.event-bookings.destroy', $eventBooking->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('event_booking_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.event-bookings.massDestroy') }}",
...
@endcan
  ...
  let table = $('.datatable-EventBooking:not(.ajaxTable)').DataTable({ buttons: dtButtons })
