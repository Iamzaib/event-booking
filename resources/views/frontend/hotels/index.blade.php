        ...
        <div class="col-md-12">
            @can('hotel_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.hotels.create') }}">
                            ...
                            <tbody>
                                @foreach($hotels as $key => $hotel)
                                    <tr data-entry-id="{{ $hotel->id }}">
                                        ...
                                        <td>
                                            @can('hotel_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.hotels.show', $hotel->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('hotel_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.hotels.edit', $hotel->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('hotel_delete')
                                                <form action="{{ route('frontend.hotels.destroy', $hotel->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('hotel_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.hotels.massDestroy') }}",
