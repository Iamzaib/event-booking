...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.eventAddon.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.event-addons.update", [$eventAddon->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="addon_title">{{ trans('cruds.eventAddon.fields.addon_title') }}</label>
                <input class="form-control {{ $errors->has('addon_title') ? 'is-invalid' : '' }}" type="text" name="addon_title" id="addon_title" value="{{ old('addon_title', $eventAddon->addon_title) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('addon_details') ? 'is-invalid' : '' }}" type="text" name="addon_details" id="addon_details" value="{{ old('addon_details', $eventAddon->addon_details) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('addon_price') ? 'is-invalid' : '' }}" type="number" name="addon_price" id="addon_price" value="{{ old('addon_price', $eventAddon->addon_price) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\EventAddon::STATUS_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('status') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $eventAddon->status) === (string) $key ? 'checked' : '' }}>
