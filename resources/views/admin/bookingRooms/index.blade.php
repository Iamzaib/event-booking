...
@can('booking_room_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.booking-rooms.create') }}">
                        ...
                        <tr data-entry-id="{{ $bookingRoom->id }}">
                            ...
                            <td>
                                @can('booking_room_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.booking-rooms.show', $bookingRoom->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('booking_room_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.booking-rooms.edit', $bookingRoom->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('booking_room_delete')
                                    <form action="{{ route('admin.booking-rooms.destroy', $bookingRoom->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('booking_room_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.booking-rooms.massDestroy') }}",
