    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.payments.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('payment_event') ? 'is-invalid' : '' }}" name="payment_event_id" id="payment_event_id" required>
                    @foreach($payment_events as $id => $entry)
                        <option value="{{ $id }}" {{ old('payment_event_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('payment_user') ? 'is-invalid' : '' }}" name="payment_user_id" id="payment_user_id" required>
                    @foreach($payment_users as $id => $entry)
                        <option value="{{ $id }}" {{ old('payment_user_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('payment_booking') ? 'is-invalid' : '' }}" name="payment_booking_id" id="payment_booking_id" required>
                    @foreach($payment_bookings as $id => $entry)
                        <option value="{{ $id }}" {{ old('payment_booking_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount_total') ? 'is-invalid' : '' }}" type="number" name="amount_total" id="amount_total" value="{{ old('amount_total', '') }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount_balance') ? 'is-invalid' : '' }}" type="number" name="amount_balance" id="amount_balance" value="{{ old('amount_balance', '0') }}" step="0.01">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Payment::PAYMENT_METHOD_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('payment_method') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="payment_method_{{ $key }}" name="payment_method" value="{{ $key }}" {{ old('payment_method', 'CC') === (string) $key ? 'checked' : '' }} required>
