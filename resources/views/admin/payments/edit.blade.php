    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.payments.update", [$payment->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount_total') ? 'is-invalid' : '' }}" type="number" name="amount_total" id="amount_total" value="{{ old('amount_total', $payment->amount_total) }}" step="0.01" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('amount_balance') ? 'is-invalid' : '' }}" type="number" name="amount_balance" id="amount_balance" value="{{ old('amount_balance', $payment->amount_balance) }}" step="0.01">
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Payment::PAYMENT_METHOD_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('payment_method') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="payment_method_{{ $key }}" name="payment_method" value="{{ $key }}" {{ old('payment_method', $payment->payment_method) === (string) $key ? 'checked' : '' }} required>
