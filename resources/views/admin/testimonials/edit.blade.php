    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.testimonials.update", [$testimonial->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('review_text') ? 'is-invalid' : '' }}" type="text" name="review_text" id="review_text" value="{{ old('review_text', $testimonial->review_text) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ratings') ? 'is-invalid' : '' }}" type="text" name="ratings" id="ratings" value="{{ old('ratings', $testimonial->ratings) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('event_trip_booking') ? 'is-invalid' : '' }}" name="event_trip_booking_id" id="event_trip_booking_id" required>
                    @foreach($event_trip_bookings as $id => $entry)
                        <option value="{{ $id }}" {{ (old('event_trip_booking_id') ? old('event_trip_booking_id') : $testimonial->event_trip_booking->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('review_date') ? 'is-invalid' : '' }}" type="text" name="review_date" id="review_date" value="{{ old('review_date', $testimonial->review_date) }}" required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('featured') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="featured" id="featured" value="1" {{ $testimonial->featured || old('featured', 0) === 1 ? 'checked' : '' }}>
