...
@can('hotel_room_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.hotel-rooms.create') }}">
                ...
                <tbody>
                    @foreach($hotelRooms as $key => $hotelRoom)
                        <tr data-entry-id="{{ $hotelRoom->id }}">
                            ...
                            <td>
                                @can('hotel_room_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.hotel-rooms.show', $hotelRoom->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('hotel_room_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.hotel-rooms.edit', $hotelRoom->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('hotel_room_delete')
                                    <form action="{{ route('admin.hotel-rooms.destroy', $hotelRoom->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('hotel_room_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.hotel-rooms.massDestroy') }}",
