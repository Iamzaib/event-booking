...
class UsersController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        //dd($queries);
//        if(count($where)>0){
//            $users=User::with(['city', 'state', 'country', 'roles', 'media'])->whereHas('roles', function ($q) { return $q->where('title', 'User'); })->where($where);
//        }
...
//        dd($users_countries);;
        //$users_countries=Arr::join($users_countries->toArray(),',');//dd($users_countries);
////        $filters['countries'] = Country::whereIn('id', function ($query) {
//            $query->select('country_id')->from('users')->groupby('country_id');
...
//        dd($queries);
        ...
        $sort .= '-' . $sort_type;
        return view('admin.users.index', compact('users','sort','per_page','sort_type','filters','admin'));
    }
